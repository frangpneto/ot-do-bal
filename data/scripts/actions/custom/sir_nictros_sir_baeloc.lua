local config = {
	...
	daily = true,
	centerRoom = Position(33425, 31438, 13),
	playerPositions = {
	...
	teleportPosition = Position(33425, 31443, 13),
	bossPosition1 = Position(33422, 31437, 13),
	bossPosition2 = Position(33428, 31437, 13),
	specPos = Position(33426, 31409, 13)
}
	...
	if item.itemid == config.leverId then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.GraveDanger.SirsTimer) > os.time() then
					player:getPosition():sendMagicEffect(CONST_ME_POFF)
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You or a member in your team have to wait ".. config.timeToFightAgain .."  hours to face the Bosses again!")
					return true
				end
		...
		for i = 1, #specs do
			...
			if spec:isPlayer() then
				player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "There's someone fighting with the Bosses")
				return true
			end
			spec:remove()
		end
		...
		Game.createMonster(config.bossName1, config.bossPosition1)
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.GraveDanger.SirsTimer, os.time() + config.timeToFightAgain * 60 * 60) -- 20 hours
