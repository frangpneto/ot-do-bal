local config = {
	...
	daily = true,
	centerRoom = Position(33456, 31471, 13),
	playerPositions = {
	...
	teleportPosition = Position(33459, 31475, 13),
	bossPosition = Position(33455, 31470, 13),
	specPos = Position(33456, 31497, 13)
}
	...
	if item.itemid == config.leverId then
		...
		for i = 1, #config.playerPositions do
			...
			if participant and participant:isPlayer() then
				...
				if config.daily and participant:getStorageValue(Storage.GraveDanger.DukeTimer) > os.time() then
					player:getPosition():sendMagicEffect(CONST_ME_POFF)
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You or a member in your team have to wait ".. config.timeToFightAgain .."  hours to face Duke Krule again!")
					return true
				end
		...
		for i = 1, #team do
			...
			-- Assign boss timer
			team[i]:setStorageValue(Storage.GraveDanger.DukeTimer, os.time() + config.timeToFightAgain * 60 * 60) -- 20 hours
